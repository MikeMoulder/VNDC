...
function CopyButton({ value }: { value: string }) {
  ...
  return (
    <button
      onClick={copy}
      className="rounded-lg border border-white/10 p-2 text-slate-400 transition-all hover:border-white/20 hover:bg-white/5 hover:text-white"
      type="button"
...
export function ProofDrawer({ verdict }: { verdict: VerdictResponse }) {
  ...
  return (
    <div className="glass-card overflow-hidden">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between p-5 text-left transition-colors hover:bg-white/[0.02] md:p-6"
        type="button"
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold tracking-tight">Verification Details</h3>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <div className="space-y-3 border-t border-white/[0.06] p-5 md:p-6">
              {/* Receipt ID */}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{call.chain}</p>
                        </div>
